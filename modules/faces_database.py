...
class FacesDatabase:
    ...
    def __init__(self, path, face_identifier, landmarks_detector, face_detector=None, no_show=False):
        ...
        if osp.isdir(path):
            ...
            log.error('Wrong face images database path. Expected a path to the ' \
                      'directory containing %s files, but got \'{}\''.format(
        ...
        for num, path in enumerate(paths):
            ...
            assert len(image.shape) == 3, 'Expected an input image in (H, W, C) format'
            ...
            if face_detector:
                ...
                if len(rois) < 1:
                    log.warning('Not found faces on the image \'{}\''.fomrat(path))
            else:
    ...
    def dump_faces(self, image, desc, name):
        ...
        if osp.exists(filename):
            log.warning('File with the same name already exists at {}. Skipping'.format(self.fg_path))
